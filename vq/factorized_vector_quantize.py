...
class FactorizedVectorQuantize(nn.Module):
    def __init__(self, dim, codebook_size, codebook_dim, commitment, **kwargs):
        ...
        self.codebook_weight = kwargs.get("codebook_weight", 4.0)
    ...
    def forward(self, z):
        ...
        if self.training:
            ...
            utilization = unique_codes.numel() / self.codebook_size
        else:
            utilization = torch.tensor(0.0, device=z.device)

        if self.training:
            commitment_loss = F.mse_loss(z_e, z_q.detach(), reduction='none').mean([1, 2]) * self.commitment
            codebook_loss = F.mse_loss(z_q, z_e.detach(), reduction='none').mean([1, 2]) * self.codebook_weight
        ...
        return z_q, indices, commit_loss, utilization
