...
def train(cfg):
    ...
    for wav_path in tqdm(wav_paths):
        ...
        with torch.no_grad():
            vq_emb = lm.model['CodecEnc'](wav.unsqueeze(1), lengths=lengths)
            vq_post_emb, vq_code, _ = lm.model['generator'](vq_emb, vq=True)
            recon  = lm.model['generator'](vq_post_emb, vq=False, lengths=lengths).squeeze(1) # [B, T]
        recon = recon.squeeze(0)[:lengths[0]].detach().cpu().numpy()
