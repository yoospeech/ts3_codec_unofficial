...
class CodecLightningModule(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        vq_emb = self.model['CodecEnc'](wav.unsqueeze(1), lengths=lengths)
        vq_post_emb, vq_code, vq_loss, utilization = self.model['generator'](vq_emb, vq=True)
        # log codebook utilization (mean over quantizers if multi)
        if isinstance(utilization, torch.Tensor) and utilization.ndim > 0:
            util_val = utilization.float().mean()
        else:
            util_val = utilization
        self.log('codebook_utilization', util_val, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        y_ = self.model['generator'](vq_post_emb, vq=False, lengths=lengths) # [B, 1, T]
        y = wav.unsqueeze(1)
        output = {
            'gt_wav': y,
            'gen_wav': y_,
    ...
    def inference(self, wav):
        lengths = torch.LongTensor([wav.shape[1]] * wav.shape[0]).to(wav.device)
        vq_emb = self.model['CodecEnc'](wav.unsqueeze(1), lengths=lengths)
        vq_post_emb, vq_code, vq_loss = self.model['generator'](vq_emb, vq=True)
        y_ = self.model['generator'](vq_post_emb, vq=False, lengths=lengths).squeeze(1) # [B, T]
        return y_
